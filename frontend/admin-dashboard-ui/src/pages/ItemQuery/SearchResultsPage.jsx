...
const SearchResultsPage = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    minPrice: "",
    maxPrice: "",
    brand: "", // Added brand to filters object for consistency
  });
  ...
  const handlePriceFilter = (e) => {
    ...
    const brand = filters.brand || params.get("brand") || ""; // Preserve brand
  ...
  const handleBrandFilter = (brandName) => {
    // Toggle selection
    const selected = filters.brand === brandName ? "" : brandName;
  ...
  const partsToDisplay = carParts.filter((part) => {
    ...
    // Apply brand filter if set - use CarModel.brand_name
    if (filters.brand && part.CarModel?.brand_name !== filters.brand) {
      return false;
    }
        ...
        {!hasSearched && !searchTerm ? (
          ...
          <>
            <h2 className="text-lg font-medium mb-4">
              ...
              {filters.brand && ` for ${filters.brand}`}
